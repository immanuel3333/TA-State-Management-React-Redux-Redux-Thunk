...
const cartReducer = (state = initState, action) => {
  ...
  if (action.type === REMOVE_ITEM) {
    ...
    const nextState = produce(state, (draf) => {
      const id = action.payload;
      let itemToRemove = draf.addedItems.find((item) => item.id === id);
      if (itemToRemove) {
        let new_items = draf.addedItems.filter((item) => item.id !== id);
        let newTotal = draf.total - itemToRemove.price * itemToRemove.quantity;
        return {
          ...state,
  ...
  if (action.type === SUB_QUANTITY) {
    ...
    const nextState = produce(state, (draf) => {
      const id = action.payload;
      const findItem = draf.addedItems.find((item) => item.id === id);
      if (findItem.quantity === 1) {
        let newItem = draf.addedItems.filter((item) => item.id !== id);
        return {
          ...state,
          addedItems: newItem,
        };
        ...
        findItem.quantity--;
        draf.total = draf.total - findItem.price;
      }
    });
